...
func (a Authentication) InitAuthenticationController(m models.MuxServer) {
	m.Mux.Route(m.Endpoint+"/auth/login", func(r chi.Router) {
		r.Get("/", a.Login)
	})
...
func (a Authentication) Login(w http.ResponseWriter, r *http.Request) {
	...
	a.c.SetCache(fmt.Sprintf("%s:%s", strconv.Itoa(int(user.ID)), user.Username), token)
